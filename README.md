...
Главная страница открывается по корневому URL и через логотип MyHealth; неизвестные
frontend-маршруты перенаправляются на неё. Дашборд использует обязательный
включительный диапазон дат и возвращает баланс калорий по дням с записями
журнала, изменение веса между первым и последним измерениями и топ-10 видов
активности. Без пользовательского дефолтного лимита график калорий не строится.
...
продукта в тот же приём пищи заменяет его вес. Итоги КБЖУ рассчитываются по
актуальным данным продуктов; проценты Б/Ж/У — от общей массы этих трёх
макронутриентов. В журнале можно задать активные калории для выбранной
даты: они используются как дневной лимит вместо дефолта из настроек. Удаление
дневного значения возвращает расчёт к дефолту; если нет обоих значений, прогресс
не рассчитывается.
